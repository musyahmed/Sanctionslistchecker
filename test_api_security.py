...
try:
    from main_api import app, mock_user_db, token_blocklist, UserInDB # Assuming UserInDB might be useful
    ...
    # from main_api import app, mock_user_db, token_blocklist 
    raise
...
def reset_global_state():
    """Resets mock_user_db and token_blocklist for test isolation."""
    mock_user_db.clear()
    token_blocklist.clear()
    # print("Global state (mock_user_db, token_blocklist) reset.") # For debugging
...
class TestAPISecurity(unittest.TestCase):
    ...
    def test_access_protected_endpoint_no_token(self):
        ...
        reset_global_state() # Ensure clean state
        # Need a user in mock_vector_db_data for /chat to not 404 on data, but auth should fail first
    ...
    def test_logout_and_token_revocation(self):
        ...
        self.assertGreater(len(token_blocklist), 0, "Token JTI should be in blocklist after logout")
    ...
    def test_chat_input_validation(self):
        ...
        # The user_id for chat must exist in agent_module.mock_vector_db_data
        chat_user = "user123" # This user is in mock_vector_db_data
        ...
        invalid_char_query = "Hello <script>alert('XSS')</script>"
        ...
        # Valid query (should pass validation but might 404 if user data not in mock_vector_db)
        # User 'user123' is in mock_vector_db_data, so it should proceed past validation.
        valid_query = "What is my balance?"
        response_valid_q = client.post("/chat", headers=headers, json={"query": valid_query})
        # We are testing validation here, so 200 means validation passed.
        # The actual response content depends on the LLM mock or actual call.
        self.assertEqual(response_valid_q.status_code, 200) 
    ...
    def test_mfa_setup_and_login_flow(self):
        ...
        self.assertIn(mfa_user, mock_user_db)
        self.assertEqual(mock_user_db[mfa_user].mfa_secret, mfa_secret_for_testing)
        ...
        self.assertTrue(mock_user_db[mfa_user].is_mfa_enabled)
